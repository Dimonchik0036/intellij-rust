...
//        assertNotEquals(set(longArrayOf(0, 10, 12, 15)), set(longArrayOf(0, 9, 12, 15)))
    ...
    fun `test negate`() {
        assertTrue(empty().negate().isEmpty())
        assertEquals(point(RsRanges.MAX_VALUE), point(RsRanges.MIN_VALUE + 1).negate())
        assertEquals(point(RsRanges.MIN_VALUE), point(RsRanges.MIN_VALUE).negate())
        assertEquals(point(Integer.MIN_VALUE.toLong()), point(Integer.MIN_VALUE.toLong()).negate())
        assertEquals(
            point(Integer.MAX_VALUE + 1L, TyInteger.I64),
            point(Integer.MIN_VALUE.toLong(), TyInteger.I64).negate()
        )
        assertEquals(range(-200, -100), range(100, 200).negate())
        assertEquals(range(-200, 1), range(-1, 200).negate())
        assertEquals(range(-200, 200), range(-200, 200).negate())
        assertEquals(range(-200, 201), range(-201, 200).negate())
        assertEquals(all(), all().negate())
        assertEquals(
            range(100, Integer.MAX_VALUE.toLong()).union(point(Integer.MIN_VALUE.toLong())),
            range(Integer.MIN_VALUE.toLong(), -100).negate()
        )
        assertEquals(
            point(RsRanges.MAX_VALUE, TyInteger.I64).union(point(RsRanges.MIN_VALUE, TyInteger.I64)),
            range(RsRanges.MIN_VALUE, RsRanges.MIN_VALUE + 1, TyInteger.I64).negate()
        )
        assertEquals(
            range(100, Integer.MAX_VALUE + 1L, TyInteger.I64),
            range(Integer.MIN_VALUE.toLong(), -100, TyInteger.I64).negate()
        )
        val set = range(-900, 1000).subtract(range(-800, -600)).subtract(range(-300, 100)).subtract(range(500, 700))
        assertEquals("{-900..-801, -599..-301, 101..499, 701..1000}", set.toString())
        assertEquals("{-1000..-701, -499..-101, 301..599, 801..900}", set.negate().toString())
    }
